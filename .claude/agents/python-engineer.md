...
full conventions (`<root>` = the repo root; see Environment & Tooling). The essentials:

## Environment & Tooling

- Workspace root `<root>` = the repository root, injected into Bash as **`$PROJECT_ROOT`**. Venv:
  `$PROJECT_ROOT/.venv`. In Bash commands use `$PROJECT_ROOT`; for Read/Write tool paths (no env-var
  expansion) resolve the literal first with `/opt/homebrew/opt/coreutils/libexec/gnubin/echo "$PROJECT_ROOT"`.
...
- Python **3.11+**, **pydantic v2** (`pydantic>=2.10.0,<3.0.0`).
- After every change, the code must be **ruff-clean** and **ty-clean**:
  ...
  ```
  (In Bash commands, `$PROJECT_ROOT` expands to `<root>`.)
...
### 3. pydantic v2 + ty
- Use pydantic **v2** idioms: `field_validator`/`model_validator` (with `mode='before'`/`mode='after'`),
  `ValidationInfo`, `model_config = ConfigDict(...)`, `model_rebuild()`, `Field(...)`, and
  `__get_pydantic_core_schema__` + `pydantic_core.core_schema` for custom field types. Do **not**
  reintroduce removed v1 APIs (`validator`, `pre=`/`always=`, `ModelField`, inner `class Config`,
  `update_forward_refs()`, `__get_validators__`, `.dict()`/`.json()`).
- Keep the tree **ty-clean**. Prefer a real type fix (accurate annotations, narrowing, `cast`) over a
  ...
  is acceptable only for constructs with known, uniform type-checker friction (e.g. some pydantic
  dynamic-model / forward-ref lines). **Pyright-style codes (`# type: ignore[reportXxx]`) do not work in ty** —
...
- any pydantic v2 model or `model_config` → Pattern 3 (`ConfigDict`, alias generator, `validate_assignment`, `extra`)
- a custom input field type or validator → Patterns 4 / 4a / 4b (`__get_pydantic_core_schema__` chain, `string()`-style
...
- self-referential / recursive models → Pattern 6 (`model_rebuild()`)
