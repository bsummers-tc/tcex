...
4. The plan body must include, in order:
   ...
   - `## Human Acceptance Criteria` — **always present.** A short list of **manual steps a human runs
     ...
     - One or two steps per command / user-facing behavior; each step = the exact command to run (or
       ...
       checks them later.
     - Order them: read-only / `--help` first, then dry-run / preview, then write / regenerate
     ...
     - Make them runnable as written: real command/flag names and any prerequisite (venv synced,
       submodule pointer bumped, `.env` creds present for a regeneration). Examples appropriate to this
       framework:
       - `- [ ] Run \`"$PROJECT_ROOT/.venv/bin/pytest" tests/util -k <new_test>\` → the new test passes.`
       - `- [ ] Dry-run the change in a Python REPL (\`"$PROJECT_ROOT/.venv/bin/python" -c "..."\`) and
         confirm the new field/behavior is present.`
       - `- [ ] (If generated code changed) source \`.env\`, run
         \`"$PROJECT_ROOT/.venv/bin/python" "$PROJECT_ROOT/tcex/api/tc/v3/_gen/_gen.py" all\`, then
         \`"$PROJECT_ROOT/.venv/bin/pre-commit" run --all-files\` → the diff matches the intended change.`
       - `- [ ] Cleanup: revert any throwaway REPL artifacts / generated scratch files.`
...
5. **Adversarial plan review (opt-in gate).** Once the plan body is fully written (through
   ...
   **iterate to convergence:**
   - Resolve **every 🔴 Critical and 🟠 Significant finding** by editing the plan. Before accepting or
     rejecting any contested factual claim (library, API behavior, generated-code surface, pydantic
     API), **verify it against a primary source** (the codebase, official docs via WebFetch, or the
     `claude-code-guide` agent) — do not fix on faith, and do not silently drop a finding you believe
     is wrong without verifying.
...
Constraints: [pydantic v1, ty-clean, submodule commit rules, no hand-editing generated files, etc.]
...
(you never stage or commit), bump a submodule pointer, regenerate `tcex/api/tc/v3` against a chosen
`TC_API_PATH` server (requires `.env` creds), `uv sync` after a dependency change, or perform manual QA.
